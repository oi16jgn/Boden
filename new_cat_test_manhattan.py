...
def amount_of_matches(applicants_df, companies_df, results_df):
    ...
    for index_applicant, result_row in results_df.iterrows():
        applicant_row = applicants_df.loc[applicants_df['Submission ID'] == result_row['ID of Applicant']].iloc[0]
        company_ids = result_row['Top 20 Similar IDs from Companies']
        max_matches = count_title_matches(applicant_row, companies_df)
        matches_count = 0
        for company_id in company_ids:
            if matches_count >= max_matches:
                break
            row_company = companies_df.loc[companies_df['Submission ID'] == company_id].iloc[0]
            if check_match(applicant_row, row_company):
                amount += 1
            matches_count += 1
    return amount

def count_title_matches(applicant_row, companies_df):
    count = 0
    for index_company, row_company in companies_df.iterrows():
        if check_match(applicant_row, row_company):
            count += 1
